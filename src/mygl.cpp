...
MyGL::MyGL(QWidget* parent)
    ...
    , m_progPathTracer(this)
    , m_progDisplay(this)
    , m_progCubemapConversion(this)
    , m_renderPassOutputFBs{FrameBuffer2D(this,
                                          this->width(),
                                          this->height(),
...
void MyGL::initializeGL()
{
    ...
    this->loadJSON(":/jsons/PT_cornellBoxPointLight.json");
...
void MyGL::paintGL()
{
    if (m_progPathTracer.m_isReloading) {
        return;
    }
    ...
    for (unsigned int i = 0; i < m_scene.meshes.size(); ++i) {
        ...
        m_progPathTracer.setUnifArrayInt("u_TriangleStorageSamplers",
                                         i,
                                         m->trianglesAsTexture->m_associatedTextureSlot);
    }
...
{
    std::vector<const char*> fragfile_sections = {":/glsl/pathtracer.sampleWarping.glsl",
