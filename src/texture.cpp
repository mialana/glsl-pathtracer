...
Texture::Texture(OpenGLContext* context, int arrayIdx, int associatedTexSlot)
    : m_isCreated(false)
    , m_uniformArrayIndex(arrayIdx)
    , m_associatedTextureSlot(associatedTexSlot)
    , context(context)
    , m_textureHandle(0)
    , m_texturePath("")
{}
...
TextureTriangleStorage::TextureTriangleStorage(OpenGLContext* context,
                                               int arrayIdx,
                                               int associatedTexSlot,
                                               Mesh* mesh)
    : Texture(context, arrayIdx, associatedTexSlot)
    , representedMesh(mesh)
{}
...
void Texture2DHDR::create(bool wrap)
{
    ...
    float* data = stbi_loadf(m_texturePath.toStdString().c_str(), &width, &height, &nrComponents, 0);
    if (data) {
