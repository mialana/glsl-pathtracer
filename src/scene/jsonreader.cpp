...
JSONReader::JSONReader(OpenGLContext* context)
    : m_shapeCounts()
    , mp_context(context)
    , m_lowestUnusedArrayIdx(0)
    , m_lowestUnusedTexSlot(4)
{}

bool JSONReader::LoadSceneFromFile(QFile& file,
...
bool JSONReader::LoadGeometry(QJsonObject& geometry,
                              ...
                              Scene& scene)
{
    ...
    if (QString::compare(type, QString("Mesh")) == 0) {
        auto shape = mkU<Mesh>(this->mp_context,
                               Mesh::nextLowestSamplerIndex++,
                              ...
                              std::vector<uPtr<Texture>>* mtl_textures)
{
    ...
    if (material.contains(QString("type"))) {
    ...
    if (QString::compare(type, QString("MatteMaterial")) == 0) {
        ...
        if (material.contains(QString("textureMap"))) {
            ...
            textureMap = mkU<Texture2D>(mp_context,
                                        m_lowestUnusedArrayIdx++,
                                        m_lowestUnusedTexSlot++);
        ...
        if (material.contains(QString("normalMap"))) {
            ...
            normalMap = mkU<Texture2D>(mp_context,
                                       m_lowestUnusedArrayIdx++,
                                       m_lowestUnusedTexSlot++);
            normalMap->create(img_filepath.toStdString().c_str(), false);
        }
        auto result = mkU<Material>(Kd,
                                    sigma,
                                    -1.,
                                    DIFFUSE_REFL,
                                    textureMap.get(),
        ...
        if (material.contains(QString("roughnessMap"))) {
            ...
            roughnessMap = mkU<Texture2D>(mp_context,
                                          m_lowestUnusedArrayIdx++,
                                          m_lowestUnusedTexSlot++);
        ...
        auto result
            = mkU<Material>(Kt, 0, eta, SPEC_TRANS, textureMap.get(), normalMap.get(), nullptr);
        ...
        auto result
            = mkU<Material>(Kt, 0, eta, SPEC_GLASS, textureMap.get(), normalMap.get(), nullptr);
...
{
    Camera result;
    if (camera.contains(QString("target"))) {
        result.ref = ToVec3(camera["target"].toArray());
    }
    if (camera.contains(QString("eye"))) {
        result.eye = ToVec3(camera["eye"].toArray());
    }
    if (camera.contains(QString("worldUp"))) {
        result.world_up = ToVec3(camera["worldUp"].toArray());
    }
    if (camera.contains(QString("width"))) {
        result.width = camera["width"].toDouble();
    }
    if (camera.contains(QString("height"))) {
        result.height = camera["height"].toDouble();
    }
    if (camera.contains(QString("fov"))) {
        result.fovy = camera["fov"].toDouble();
    }
    if (camera.contains(QString("nearClip"))) {
        result.near_clip = camera["nearClip"].toDouble();
    }
    if (camera.contains(QString("farClip"))) {
        result.far_clip = camera["farClip"].toDouble();
    }
...
Transform JSONReader::LoadTransform(QJsonObject& transform)
{
    Vector3f t, r, s;
    s = Vector3f(1, 1, 1);
    if (transform.contains(QString("translate"))) {
        t = ToVec3(transform["translate"].toArray());
    }
    if (transform.contains(QString("rotate"))) {
        r = ToVec3(transform["rotate"].toArray());
    }
    if (transform.contains(QString("scale"))) {
        s = ToVec3(transform["scale"].toArray());
    }
