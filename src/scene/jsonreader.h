...
enum ShapeType { SQUARE, SPHERE, BOX, DISC, TRIANGLE };
...
public:
    ...
    bool LoadGeometry(QJsonObject& geometry,
                      const std::map<std::string, uPtr<Material>>& mtl_map,
                      const QString& local_path,
                      Scene& scene);
    bool LoadLights(QJsonObject& geometry,
                    const std::map<std::string, uPtr<Material>>& mtl_map,
                    const QString& local_path,
                    Scene& scene);
    bool LoadMaterial(QJsonObject& material,
                      QString local_path,
                      std::map<std::string, uPtr<Material>>* mtl_map,
                      std::vector<std::unique_ptr<Texture>>* mtl_textures);
