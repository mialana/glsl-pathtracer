...
{
    return writeTransform(transform) + ", " + writeInt(ID) + ", " + material->toGLSL();
}

QString Sphere::toGLSL(int ID) const
{
    return "Sphere(" + writeVec3(pos) + ", " + writeFloat(radius) + ", " + writeMembers(ID) + ")";
}

QString Box::toGLSL(int ID) const
{
    return "Box(" + writeVec3(minCorner) + ", " + writeVec3(maxCorner) + ", " + writeMembers(ID)
           + ")";
}

QString RectangleShape::toGLSL(int ID) const
{
    return "Rectangle(" + writeVec3(pos) + ", " + writeVec3(nor) + ", " + writeVec2(halfSideLengths)
           + ", " + writeMembers(ID) + ")";
}

QString Mesh::toGLSL(int ID) const
{
    return "Mesh(" + writeInt(triangleSamplerIndex) + ", " + writeInt(triangleStorageSideLen) + ", "
           + writeInt(triangles.size()) + ", " + writeMembers(ID) + ")";
}
    ...
    if (errors.size() == 0) {
        ...
        for (unsigned int i = 0; i < shapes.size(); i++) {
            ...
            for (unsigned int j = 0; j < indices.size(); j += 3) {
                glm::vec3 p1(positions[indices[j] * 3],
                             positions[indices[j] * 3 + 1],
                             positions[indices[j] * 3 + 2]);
                glm::vec3 p2(positions[indices[j + 1] * 3],
                             positions[indices[j + 1] * 3 + 1],
                             positions[indices[j + 1] * 3 + 2]);
                glm::vec3 p3(positions[indices[j + 2] * 3],
                ...
                if (normals.size() > 0) {
                    glm::vec3 n1(normals[indices[j] * 3],
                                 normals[indices[j] * 3 + 1],
                                 normals[indices[j] * 3 + 2]);
                    glm::vec3 n2(normals[indices[j + 1] * 3],
                                 normals[indices[j + 1] * 3 + 1],
                                 normals[indices[j + 1] * 3 + 2]);
                    glm::vec3 n3(normals[indices[j + 2] * 3],
                                 normals[indices[j + 2] * 3 + 1],
                                 normals[indices[j + 2] * 3 + 2]);
                    t.nor[0] = n1;
                    t.nor[1] = n2;
                    t.nor[2] = n3;
                }
